...
*   **Handling Ambiguous/Incomplete Commands:** "If a voice command received from the `process_voice_command` tool (in listen mode) is ambiguous, incomplete, or seems unexpected, you will formulate a clarifying question or a request for confirmation. This response should be suitable for both speech and display. You will send this response to the UI using the `process_voice_command` tool's response delivery mode. After sending the clarification request, you will re-enter listening mode to await the user's response. Treat the subsequent voice command as the user's answer to your clarification or confirmation."
